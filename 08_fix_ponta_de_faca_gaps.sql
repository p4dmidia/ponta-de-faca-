-- ============================================================================
-- SCRIPT DE ATUALIZAÇÃO: CORREÇÃO DE LACUNAS DO CLUBE E PDV (CONSOLIDADO)
-- ============================================================================

-- 1. TRIGGER DE CONTROLE DE VAGAS DO CLUBE (LIMITADO A 50 ASSINANTES ATIVOS)
CREATE OR REPLACE FUNCTION public.check_subscription_slots()
RETURNS trigger AS $$
DECLARE
...
BEGIN
  ...
  IF (TG_OP = 'INSERT' AND NEW.status = 'Pago') OR 
     (TG_OP = 'UPDATE' AND (OLD.status IS NULL OR OLD.status != 'Pago') AND NEW.status = 'Pago') THEN
    
    -- Verificar se o pedido contém algum produto da categoria Planos ou com nome que indique assinatura
    FOR v_item IN 
      SELECT product_id FROM public.order_items WHERE order_id = NEW.id 
    LOOP
      SELECT * INTO v_product FROM public.products WHERE id = v_item.product_id;
      IF v_product IS NOT NULL THEN
        SELECT name INTO v_category_name FROM public.product_categories WHERE id = v_product.category_id;
    ...
    IF v_has_sub THEN
      -- Contar outros membros ativos na mesma organização
      ...
      IF v_active_subs >= 50 THEN
        RAISE EXCEPTION 'Clube completo: O limite de 50 membros ativos já foi atingido (% membros ativos). O pedido não pode ser ativado.', v_active_subs;
      END IF;
    END IF;
  END IF;
  RETURN NEW;
...
DROP TRIGGER IF EXISTS trigger_check_subscription_slots ON public.orders;
CREATE TRIGGER trigger_check_subscription_slots
  BEFORE INSERT OR UPDATE ON public.orders
  FOR EACH ROW
  EXECUTE FUNCTION public.check_subscription_slots();


-- 2. AJUSTAR GATILHO DE CADASTRO PARA CAPTURAR PARCEIRO (PDV) COMO SPONSOR
 ...
 BEGIN
   ...
   IF v_full_name = '' THEN v_full_name := v_login; END IF;
    ...
    IF v_sponsor_code IS NOT NULL THEN
      -- Se for indicação de PDV, do tipo pdv-X
      IF LOWER(v_sponsor_code) LIKE 'pdv-%' THEN
        DECLARE
          v_pdv_id bigint;
          v_pdv_email text;
        BEGIN
          v_pdv_id := split_part(v_sponsor_code, '-', 2)::bigint;
          SELECT email INTO v_pdv_email FROM public.companies WHERE id = v_pdv_id;
          
          IF v_pdv_email IS NOT NULL THEN
            SELECT id, user_id INTO v_sponsor_affiliate_id, v_sponsor_user_id
            FROM public.affiliates 
            WHERE LOWER(email) = LOWER(v_pdv_email)
              AND organization_id = v_org_id
            LIMIT 1;
          END IF;
        EXCEPTION WHEN OTHERS THEN
          v_sponsor_affiliate_id := NULL;
          v_sponsor_user_id := NULL;
        END;
      ELSE
        -- Fluxo padrão de afiliado normal
        SELECT id, user_id INTO v_sponsor_affiliate_id, v_sponsor_user_id
        FROM public.affiliates 
...
-- 3. ATUALIZAR DISTRIBUIÇÃO DE COMISSÕES (RESOLUÇÃO DE REFERÊNCIA DE PDV EM VENDAS)
...
BEGIN
    ...
    IF (OLD.status IS NULL OR OLD.status != 'Pago') AND NEW.status = 'Pago' THEN
        ...
        IF NEW.referral_code IS NOT NULL AND NEW.referral_code != '' THEN
            -- Se for indicação de PDV (ex: pdv-2)
            IF LOWER(NEW.referral_code) LIKE 'pdv-%' THEN
              DECLARE
                v_pdv_id bigint;
                v_pdv_email text;
              BEGIN
                v_pdv_id := split_part(NEW.referral_code, '-', 2)::bigint;
                SELECT email INTO v_pdv_email FROM public.companies WHERE id = v_pdv_id;
...
-- 4. FUNÇÃO RPC PARA CONFIRMAÇÃO DE RETIRADA E COMISSIONAMENTO AUTOMÁTICO DO PDV
CREATE OR REPLACE FUNCTION public.confirm_pdv_pickup(p_order_id text, p_company_id bigint)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  -- 2. Verificar se o método é retirada e se o status é válido
  IF v_order.shipping_method != 'Retirada em PDV' THEN
    RETURN jsonb_build_object('success', false, 'message', 'Este pedido não é de retirada local.');
  END IF;
  
  IF v_order.status = 'Entregue' THEN
    RETURN jsonb_build_object('success', false, 'message', 'Este pedido já foi retirado anteriormente.');
  END IF;
  
  IF v_order.status != 'Enviado' AND v_order.status != 'Pago' THEN
    RETURN jsonb_build_object('success', false, 'message', 'O pagamento deste pedido ainda não foi confirmado.');
  END IF;
  
  -- 3. Buscar a empresa
  SELECT * INTO v_company FROM public.companies WHERE id = p_company_id;
  IF v_company IS NULL THEN
    RETURN jsonb_build_object('success', false, 'message', 'PDV não encontrado.');
  END IF;
  
  -- 4. Dar baixa no estoque
  v_new_stock := GREATEST(0, v_company.stock_quantity - 1);
  ...
  -- 6. Encontrar o usuário associado à loja
  SELECT id INTO v_user_id FROM public.user_profiles WHERE LOWER(email) = LOWER(v_company.email) LIMIT 1;
  
  IF v_user_id IS NOT NULL THEN
    -- Garantir que a carteira existe
    INSERT INTO public.user_settings (user_id, organization_id, total_earnings, available_balance, frozen_balance, is_active_this_month, created_at, updated_at)
    VALUES (v_user_id, v_company.organization_id, 0, 0, 0, true, now(), now())
    ON CONFLICT (user_id) DO NOTHING;
    
    -- Creditar comissão de retirada
    UPDATE public.user_settings 
    SET 
        total_earnings = total_earnings + v_pickup_fee,
        available_balance = available_balance + v_pickup_fee,
        updated_at = now()
    WHERE user_id = v_user_id;
    
    -- Inserir extrato de comissão
    INSERT INTO public.commissions (
        organization_id,
        user_id,
        order_id,
        ...
        1,
        'pdv_pickup',
        'Comissão de entrega/retirada local - Pedido ' || p_order_id
    );
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'commission_earned', v_pickup_fee
  );
END;
$$;
