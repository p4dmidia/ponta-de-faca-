...
logins_to_check = ["sandrakarla123", "jairlima123", "Carvalho10"]

for login in logins_to_check:
    print(f"\n--- CHECKING LOGIN: {login} ---")
    affs = query_table("affiliates", f"referral_code=ilike.{login}")
    print(f"Found in affiliates: {len(affs)}")
    for a in affs:
        print(f"  Org: {a['organization_id']}, Email: {a['email']}")
