...
# Check withdrawals for any recent activity or unusual types
withdrawals = get_data("withdrawals?order=created_at.desc&limit=20")
print(f"Recent withdrawals: {json.dumps(withdrawals, indent=2)}")

# Check for any order items with 0.5 price
order_items_05 = get_data("order_items?unit_price=eq.0.5")
print(f"Order items with 0.5: {json.dumps(order_items_05, indent=2)}")
